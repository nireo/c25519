...
static inline point *point_scalar_mult(point *v, const scalar *x,
                                       const point *q) {
 ...
 */
static inline point *point_vartime_double_scalar_base_mult(point *v,
                                                           const scalar *a,
                                                           const point *A,
                                                           const scalar *b) {
  ...
  int8_t a_naf[256];
  int8_t b_naf[256];
  if (scalar_non_adjacent_form(a_naf, a, 5) != 0 ||
      scalar_non_adjacent_form(b_naf, b, 8) != 0) {
    return point_set_identity(v);
  }
