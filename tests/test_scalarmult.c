...
static int test_scalar_mult_vs_dalek(void) {
  static const uint8_t dalek_scalar_bytes[32] = {
      219, 106, 114, 9,   174, 249, 155, 89,  69,  203, 201,
      93,  92,  116, 234, 187, 78,  115, 103, 172, 182, 98,
      62,  103, 187, 136, 13,  100, 248, 110, 12,  4};
  static const uint8_t dalek_scalar_basepoint_bytes[32] = {
      0xf4, 0xef, 0x7c, 0x0a, 0x34, 0x55, 0x7b, 0x9f, 0x72, 0x3b, 0xb6,
      0x1e, 0xf9, 0x46, 0x09, 0x91, 0x1c, 0xb9, 0xc0, 0x6c, 0x17, 0x28,
      0x2d, 0x8b, 0x43, 0x2b, 0x05, 0x18, 0x6a, 0x54, 0x3e, 0x48};

  scalar s;
  if (scalar_set_canonical_bytes(&s, dalek_scalar_bytes,
                                 sizeof(dalek_scalar_bytes)) != 0) {
    fprintf(stderr, "failed to set dalek scalar\n");
    return 0;
  }
  point expected;
  if (point_set_bytes(&expected, dalek_scalar_basepoint_bytes,
                      sizeof(dalek_scalar_basepoint_bytes)) != 0) {
...
int main(void) {
  ...
  struct {
    ...
    int (*fn)(void);
      ...
      {"vartime_double_base_mult_vs_dalek",
       test_vartime_double_base_mult_vs_dalek},
      {"scalar_mult_distributes_over_add",
       test_scalar_mult_distributes_over_add},
      ...
      {"vartime_double_base_mult_matches_base_mult",
       test_vartime_double_base_mult_matches_base_mult},
  };
