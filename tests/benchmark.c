...
static void bench_sign(void *vctx) {
  ...
  const ed25519_private_key *skp =
      (const ed25519_private_key *)(const void *)&ctx->sk;
...
static void bench_verify(void *vctx) {
  ...
  const ed25519_public_key *pkp =
      (const ed25519_public_key *)(const void *)&ctx->pk;
...
int main(void) {
  ...
  {
    const ed25519_private_key *skp =
        (const ed25519_private_key *)(const void *)&sign_sk;
  ...
  if (scalar_set_uniform_bytes(&scalar_ctx.a, scalar_buf, sizeof(scalar_buf)) !=
      0) {
  ...
  if (scalar_set_uniform_bytes(&scalar_ctx.b, scalar_buf, sizeof(scalar_buf)) !=
      0) {
    fprintf(stderr, "scalar init failed\n");
    return 1;
  }
  ...
  scalar b;
  if (scalar_from_seed_uniform(&keyex_ctx.a, seed_a) != 0 ||
      scalar_from_seed_uniform(&b, seed_b) != 0) {
