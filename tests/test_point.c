...
static void encode_hex(char *out, size_t out_len, const uint8_t *in,
                       size_t in_len) {
...
static int test_non_canonical_points(void) {
  ...
  for (size_t i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) {
    uint8_t enc[32];
    uint8_t canon[32];
    if (decode_hex(tests[i].encoding, enc, sizeof(enc)) != 0 ||
        decode_hex(tests[i].canonical, canon, sizeof(canon)) != 0) {
