...
/* scalarTwo168 and scalarTwo336 are 2^168 and 2^336 modulo l in Montgomery form. */
static const scalar scalar_two_168 = { { 0x5b8ab432eac74798ULL, 0x38afddd6de59d5d7ULL,
    0xa2c131b399411b7cULL, 0x6329a7ed9ce5a30ULL } };
static const scalar scalar_two_336 = { { 0xbd3d108e2b35ecc5ULL, 0x5c3a3718bdf9c90bULL,
    0x63aa97a331b4f2eeULL, 0x3d217f5be65cb5cULL } };
...
static inline uint64_t scalar_subborrow_u64(uint64_t a, uint64_t b, uint64_t borrow, uint64_t* out)
{
...
static inline scalar scalar_new(void)
{
    return SCALAR_ZERO;
}
...
static inline scalar* scalar_subtract(scalar* s, const scalar* x, const scalar* y)
{
...
static inline scalar* scalar_multiply_add(scalar* s, const scalar* x, const scalar* y, const scalar* z)
{
    scalar zcopy = *z;
    scalar_mul(s, x, y);
...
static inline scalar* scalar_set_short_bytes(scalar* s, const uint8_t* x, size_t len)
{
    if (len >= 32) {
        return NULL;
    }
...
static inline int scalar_set_uniform_bytes(scalar* s, const uint8_t* x, size_t len)
{
    if (len != 64) {
        return -1;
    }
...
static inline int scalar_set_canonical_bytes(scalar* s, const uint8_t* x, size_t len)
{
    if (len != 32) {
        return -1;
    }
...
static inline int scalar_set_bytes_with_clamping(scalar* s, const uint8_t* x, size_t len)
{
    if (len != 32) {
        return -1;
    }
...
static inline int scalar_non_adjacent_form(int8_t out[256], const scalar* s, uint32_t w)
{
    uint8_t b[32];
    scalar_bytes(b, s);
    if (b[31] > 127) {
